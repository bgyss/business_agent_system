...
class TestFinancialDataGenerator:
    ...
    def test_generate_daily_transactions_basic(self, mock_choice, mock_normalvariate, financial_generator):
        ...
        mock_normalvariate.side_effect = [2400.0, 45.0, 50.0, 600.0]  # revenue, transaction sizes, expense
        mock_choice.side_effect = ["Walk-in Customer", "Fresh Foods Distributor"]
    ...
    def test_generate_accounts_payable(self, mock_random, financial_generator):
        ...
        with patch('simulation.financial_generator.random.randint') as mock_randint:
            mock_randint.side_effect = [2000, 1000, 9999]  # amounts and random number
    ...
    def test_generate_anomalies_empty_input(self, financial_generator):
        ...
        assert isinstance(anomalies, list)
        assert len(anomalies) >= 1  # At least 1 anomaly is generated even with empty input
