...
class TestEnhancedInventoryAgentMissingCoverage:
    ...
    async def test_process_data_different_types(self, enhanced_inventory_agent, mock_db_session):
        ...
        with patch.object(
            enhanced_inventory_agent, "_analyze_bulk_purchase_opportunities", return_value=None
        ):
            result = await enhanced_inventory_agent.process_data(data)
            assert result is None
        ...
        with patch.object(
            enhanced_inventory_agent, "_perform_expiry_intelligence", return_value=None
        ):
            result = await enhanced_inventory_agent.process_data(data)
            assert result is None
        ...
        with patch.object(
            enhanced_inventory_agent, "_analyze_supplier_performance", return_value=None
        ):
            result = await enhanced_inventory_agent.process_data(data)
            assert result is None
        ...
        with patch.object(
            enhanced_inventory_agent, "_comprehensive_inventory_analysis", return_value=None
        ):
            result = await enhanced_inventory_agent.process_data(data)
            assert result is None
    ...
    async def test_calculate_revenue_correlation_edge_cases(self, enhanced_inventory_agent):
        ...
        correlation = await enhanced_inventory_agent._calculate_revenue_correlation(
            mock_session, "ITEM001", few_movements
        )
        ...
        correlation = await enhanced_inventory_agent._calculate_revenue_correlation(
            mock_session, "ITEM001", zero_movements
        )
    ...
    async def test_calculate_revenue_correlation_exception(self, enhanced_inventory_agent):
        ...
        correlation = await enhanced_inventory_agent._calculate_revenue_correlation(
            mock_session, "ITEM001", invalid_movements
        )
        assert correlation == 0.5  # Should return default
    ...
    async def test_optimize_reorder_points_no_optimizations(
        ...
        with patch.object(
            enhanced_inventory_agent, "_calculate_optimal_reorder_point", return_value=None
        ):
            result = await enhanced_inventory_agent._optimize_reorder_points(mock_session)
            assert result is None
    ...
    async def test_calculate_optimal_reorder_point_no_forecast(
        ...
        with patch.object(enhanced_inventory_agent, "_forecast_item_demand", return_value=None):
            result = await enhanced_inventory_agent._calculate_optimal_reorder_point(
    ...
    def test_calculate_current_inventory_cost_by_id_no_item(self, enhanced_inventory_agent):
        ...
        result = enhanced_inventory_agent._calculate_current_inventory_cost_by_id(
    ...
    async def test_analyze_bulk_purchase_opportunities_no_opportunities(
        ...
        with patch.object(
            enhanced_inventory_agent, "_calculate_bulk_purchase_optimization", return_value=None
        ):
    ...
    async def test_perform_expiry_intelligence_no_analyses(self, enhanced_inventory_agent):
        ...
        perishable_item = Mock(id="ITEM001", status=ItemStatus.ACTIVE, expiry_days=7)
        mock_session.query.return_value.filter.return_value.all.return_value = [perishable_item]

        with patch.object(
            enhanced_inventory_agent, "_analyze_expiry_intelligence", return_value=None
        ):
            result = await enhanced_inventory_agent._perform_expiry_intelligence(mock_session)
            assert result is None
    ...
    async def test_analyze_expiry_intelligence_no_forecast(self, enhanced_inventory_agent):
        ...
        with patch.object(enhanced_inventory_agent, "_forecast_item_demand", return_value=None):
            result = await enhanced_inventory_agent._analyze_expiry_intelligence(
    ...
    async def test_analyze_supplier_performance_no_analyses(
        ...
        with patch.object(
            enhanced_inventory_agent, "_analyze_individual_supplier_performance", return_value=None
        ):
            result = await enhanced_inventory_agent._analyze_supplier_performance(mock_session)
            assert result is None
    ...
    async def test_comprehensive_inventory_analysis_high_issues(self, enhanced_inventory_agent):
        ...
        mock_session.query.return_value.filter.return_value.count.side_effect = [
            ...
            20,
            15,
        ]  # total, low stock (20%), overstocked (15%)
        mock_session.query.return_value.filter.return_value.scalar.side_effect = [
            50000.0,
            100.0,
        ]  # total value, low movements
    ...
    async def test_analyze_stock_movement_enhanced_unknown_type(
        ...
        movement_data = {"item_id": "ITEM001", "movement_type": "UNKNOWN_TYPE", "quantity": 10}
    ...
    async def test_analyze_consumption_movement_no_forecast(
        ...
        result = await enhanced_inventory_agent._analyze_consumption_movement(
            mock_session, sample_item, 10, None
        )
    ...
    async def test_analyze_receipt_movement_no_overstock(
        ...
        result = await enhanced_inventory_agent._analyze_receipt_movement(
            mock_session, sample_item, 20, forecast
        )
    ...
    async def test_analyze_adjustment_movement_small_adjustment(
        ...
        result = await enhanced_inventory_agent._analyze_adjustment_movement(
            mock_session, sample_item, 2, forecast
        )
        ...
        with patch.object(enhanced_inventory_agent, "_queue_analysis_message") as mock_queue:
            ...
            with patch("agents.inventory_agent_enhanced.datetime") as mock_datetime:
                mock_datetime.now.return_value = Mock(
                    hour=8, minute=15, weekday=lambda: 1
                )  # Tuesday
            ...
            with patch("agents.inventory_agent_enhanced.datetime") as mock_datetime:
                mock_datetime.now.return_value = Mock(
                    hour=9, minute=15, weekday=lambda: 0
                )  # Monday
    ...
    async def test_process_data_unknown_type(self, enhanced_inventory_agent, mock_db_session):
        ...
        with patch.object(
            enhanced_inventory_agent, "_analyze_reorder_needs_enhanced", return_value=None
        ):
            result = await enhanced_inventory_agent.process_data(data)
            assert result is None
    ...
    async def test_demand_forecasting_with_specific_item(
        ...
        data = {"type": "demand_forecast_request", "item_id": "ITEM001", "forecast_days": 21}
