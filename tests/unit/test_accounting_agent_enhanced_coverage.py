...
class TestAccountingAgentEnhancedCoverage:
    ...
    @pytest.mark.asyncio
    async def test_detect_transaction_anomalies_exception_handling(self, accounting_agent, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_analyze_time_patterns_exception_handling(self, accounting_agent, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_calculate_dynamic_confidence_exception_handling(self, accounting_agent, mock_db_session):
        ...
        analysis_data = {
            "statistics": {
                "sample_size": "invalid"  # This will cause an exception
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_get_historical_accuracy_exception_handling(self, accounting_agent, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_calculate_seasonal_consistency_exception_handling(self, accounting_agent, mock_db_session):
        ...
        result = await accounting_agent._calculate_seasonal_consistency(mock_session_instance, analysis_data)
    ...
    @pytest.mark.asyncio
    async def test_analyze_forecast_results_exception_handling(self, accounting_agent, mock_db_session):
        ...
        result = await accounting_agent._analyze_forecast_results(mock_session_instance, forecasts, 30)
    ...
    @pytest.mark.asyncio
    async def test_get_current_cash_balance_exception_handling(self, accounting_agent, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_analyze_financial_trends_exception_handling(self, accounting_agent, mock_db_session):
    ...
    async def test_calculate_period_trends_exception_handling(self, accounting_agent):
        ...
        with patch.object(accounting_agent, '_calculate_trend', side_effect=Exception("Trend calculation error")):
    ...
    async def test_calculate_trend_confidence_exception_handling(self, accounting_agent):
        ...
        trend_analysis = {
            "invalid": {
                "transaction_count": "not_a_number"
            }
        }
    ...
    async def test_process_decision_outcome_exception_handling(self, accounting_agent):
        ...
        with patch.object(accounting_agent, '_analyze_decision_patterns', side_effect=Exception("Analysis error")):
            result = await accounting_agent._process_decision_outcome(data)
    ...
    async def test_comprehensive_anomaly_detection(self, accounting_agent, mock_db_session):
        ...
        similar_transactions = [
            Mock(
                ...
                transaction_date=datetime(2023, 1, 1, 9, 0, 0)
            ),
            Mock(
            ...
            Mock(
                ...
                transaction_date=datetime(2023, 1, 1, 11, 0, 0)
            ),
            Mock(
                amount=Decimal("1300.00"),
                transaction_date=datetime(2023, 1, 1, 12, 0, 0)
            ),
            Mock(
                amount=Decimal("1400.00"),
                transaction_date=datetime(2023, 1, 1, 13, 0, 0)
            )
        ]
        ...
        for day in range(5):  # Monday to Friday
            for hour in range(9, 17):  # 9 AM to 5 PM
                similar_transactions.append(Mock(
                    transaction_date=datetime(2023, 1, 2 + day, hour, 0, 0)
                ))
    ...
    @pytest.mark.asyncio
    async def test_dynamic_confidence_calculation_comprehensive(self, accounting_agent, mock_db_session):
        """Test dynamic confidence calculation with comprehensive scenarios"""
        mock_session_instance = Mock()
        ...
        analysis_data = {
            "statistics": {
                "sample_size": 50  # High data volume
            },
    ...
    @pytest.mark.asyncio
    async def test_forecast_analysis_comprehensive_scenarios(self, accounting_agent, mock_db_session):
        """Test forecast analysis with different cash scenarios"""
        mock_session_instance = Mock()
        
        # Test scenario 1: Cash shortage predicted
        forecasts = {
            "total_forecast": -2000.0,  # Negative forecast
            "daily_forecast": -66.67
        }
    ...
    async def test_additional_edge_cases_for_coverage(self, accounting_agent, mock_db_session):
        ...
        # Setup overdue items
        overdue_receivables = [Mock(customer_name="Test", amount=Decimal("100"), due_date=date.today()-timedelta(days=40), invoice_number="INV001")]
        overdue_payables = [Mock(vendor_name="Test", amount=Decimal("50"), due_date=date.today()-timedelta(days=10), invoice_number="BILL001")]
        
        mock_session_instance.query.return_value.filter.return_value.all.side_effect = [overdue_receivables, overdue_payables]
        
        # Mock Claude to raise exception to hit line 308
        with patch.object(accounting_agent, 'analyze_with_claude', side_effect=Exception("Claude error")):
            try:
        ...
        result = await accounting_agent._calculate_seasonal_consistency(mock_session_instance, {"test": "data"})
    ...
    async def test_process_decision_outcome_needs_adjustment(self, accounting_agent):
        ...
        with patch.object(accounting_agent, '_analyze_decision_patterns', return_value=analysis_result):
            result = await accounting_agent._process_decision_outcome(data)
