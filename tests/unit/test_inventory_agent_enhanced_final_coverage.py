...
class TestEnhancedInventoryAgentFinalCoverage:
    ...
    @pytest.mark.asyncio
    async def test_forecast_item_demand_less_than_14_consumption_series(self, enhanced_inventory_agent):
    ...
    @pytest.mark.asyncio
    async def test_forecast_item_demand_without_sufficient_seasonal_data(self, enhanced_inventory_agent):
    ...
    @pytest.mark.asyncio
    async def test_bulk_purchase_optimization_minimal_savings(self, enhanced_inventory_agent, sample_item):
    ...
    async def test_analyze_expiry_intelligence_no_consumption(self, enhanced_inventory_agent):
        ...
        setattr(perishable_item, 'expiry_days', 10)
    ...
    @pytest.mark.asyncio
    async def test_analyze_individual_supplier_performance_edge_cases(self, enhanced_inventory_agent):
        ...
        mock_session.query.return_value.filter.return_value.all.return_value = [po_with_dates, po_without_expected]
        ...
        if performance is not None:
            ...
            assert performance.recommendation in ["Preferred supplier - increase business volume", 
                                                "Satisfactory supplier - maintain current relationship",
                                                "Review supplier relationship - consider alternatives"]

    @pytest.mark.asyncio
    async def test_comprehensive_inventory_analysis_multiple_issue_combinations(self, enhanced_inventory_agent):
        ...
        mock_session.query.return_value.filter.return_value.scalar.side_effect = [100000.0, 50.0]  # high value, very low movement
    ...
    async def test_periodic_check_exact_minute_boundaries(self, enhanced_inventory_agent):
        ...
        with patch.object(enhanced_inventory_agent, '_queue_analysis_message') as mock_queue:
            ...
            with patch('agents.inventory_agent_enhanced.datetime') as mock_datetime:
                mock_datetime.now.return_value = Mock(hour=4, minute=0, weekday=lambda: 2)  # Exactly 4:00 AM
            ...
            with patch('agents.inventory_agent_enhanced.datetime') as mock_datetime:
                mock_datetime.now.return_value = Mock(hour=6, minute=0, weekday=lambda: 3)  # Exactly 6:00 AM
    ...
    @pytest.mark.asyncio
    async def test_analyze_reorder_needs_enhanced_no_forecast_available(self, enhanced_inventory_agent):
    ...
    def test_weekly_seasonality_exception_handling(self, enhanced_inventory_agent):
        ...
        with patch('numpy.mean', side_effect=Exception("Numpy error")):
            seasonality = enhanced_inventory_agent._calculate_weekly_seasonality([1, 2, 3, 4, 5, 6, 7])
    ...
    @pytest.mark.asyncio
    async def test_process_data_empty_movement_data(self, enhanced_inventory_agent, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_analyze_individual_supplier_performance_poor_score(self, enhanced_inventory_agent):
        ...
        with patch.object(enhanced_inventory_agent, '_analyze_individual_supplier_performance') as mock_method:
            ...
            result = await enhanced_inventory_agent._analyze_individual_supplier_performance(mock_session, supplier)
    ...
    @pytest.mark.asyncio
    async def test_analyze_receipt_movement_without_forecast(self, enhanced_inventory_agent, sample_item):
        """Test receipt movement analysis without forecast (line 1305)"""
        mock_session = Mock()
        
        # Test overstock scenario without forecast
        result = await enhanced_inventory_agent._analyze_receipt_movement(mock_session, sample_item, 60, None)
    ...
    @pytest.mark.asyncio
    async def test_analyze_adjustment_movement_exception_path(self, enhanced_inventory_agent, sample_item):
