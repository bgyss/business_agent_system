...
from simulation.financial_generator import get_restaurant_profile, get_retail_profile
from simulation.inventory_simulator import get_restaurant_inventory_profile, get_retail_inventory_profile
from models.financial import Account, Transaction, AccountsReceivable, AccountsPayable
from models.inventory import Item, StockMovement, Supplier, PurchaseOrder
...
class TestBusinessSimulator:
    ...
    def mock_database(self):
        ...
        with patch('simulation.business_simulator.create_engine') as mock_engine, \
             patch('simulation.business_simulator.sessionmaker') as mock_sessionmaker:
    ...
    def business_simulator(self, mock_database, business_config):
        ...
        with patch('simulation.business_simulator.FinancialBase'), \
             patch('simulation.business_simulator.InventoryBase'), \
    ...
    def test_initialize_business_restaurant(self, business_simulator):
        ...
        with patch('simulation.business_simulator.get_restaurant_profile') as mock_profile, \
             patch('simulation.business_simulator.get_restaurant_inventory_profile') as mock_inv_profile, \
             patch('simulation.business_simulator.FinancialDataGenerator') as mock_fin_gen, \
             patch('simulation.business_simulator.InventorySimulator') as mock_inv_sim:
    ...
    def test_initialize_business_retail(self, business_simulator):
        ...
        with patch('simulation.business_simulator.get_retail_profile') as mock_profile, \
             patch('simulation.business_simulator.get_retail_inventory_profile') as mock_inv_profile, \
             patch('simulation.business_simulator.FinancialDataGenerator') as mock_fin_gen, \
             patch('simulation.business_simulator.InventorySimulator') as mock_inv_sim:
    ...
    def test_simulate_historical_data_success(self, business_simulator):
        ...
        mock_inventory_sim.simulate_daily_consumption.return_value = [{"movement": 1}]
        mock_inventory_sim.simulate_deliveries.return_value = [{"delivery": 1}]
        mock_inventory_sim.simulate_stock_adjustments.return_value = [{"adjustment": 1}]
    ...
    async def test_start_real_time_simulation_basic(self, business_simulator):
        ...
        task = asyncio.create_task(
            business_simulator.start_real_time_simulation(message_queue)
        )
    ...
    def test_get_simulation_status_with_data(self, business_simulator):
        ...
        # Mock query results
        mock_session.query.return_value.count.side_effect = [10, 5, 3]  # transactions, receivables, payables
