...
from exceptions import (
    ...
    AgentError, AgentInitializationError, AgentDecisionError, AgentCommunicationError,
    ClaudeAPIError,
    DatabaseError, DatabaseConnectionError, DatabaseTransactionError, 
    DatabaseIntegrityError, DataValidationError,
    ConfigurationError, ConfigFileNotFoundError, ConfigValidationError, 
    EnvironmentVariableError,
    SimulationError, SimulationInitializationError, DataGenerationError, 
    BusinessProfileError,
    ExternalServiceError, APIRateLimitError, ServiceUnavailableError,
    BusinessLogicError, InsufficientDataError, BusinessRuleViolationError, 
    InventoryError,
    FinancialError, InsufficientFundsError, CashFlowError, AccountingAnomalyError,
    DashboardError, DataVisualizationError, ReportGenerationError,
    SystemError, ResourceExhaustionError, TimeoutError,
    RecoverableError, NonRecoverableError,
    ERROR_TYPE_MAPPING, create_exception
)
...
class TestConfigurationExceptions:
    ...
    def test_config_validation_error(self):
        ...
        exc = ConfigValidationError("Missing config", config_section=section, missing_keys=missing_keys)
...
class TestBusinessLogicExceptions:
    ...
    def test_business_rule_violation_error(self):
        ...
        rule_value = 10000
        exc = BusinessRuleViolationError("Rule violated", rule_name=rule_name, rule_value=rule_value)
...
class TestSystemExceptions:
    ...
    def test_timeout_error(self):
        ...
        timeout_seconds = 30
        exc = TimeoutError("Operation timed out", operation=operation, timeout_seconds=timeout_seconds)
...
class TestRecoveryExceptions:
    ...
    def test_recoverable_error(self):
        ...
        max_retries = 5
        backoff_seconds = 2
        exc = RecoverableError("Recoverable error", max_retries=max_retries, backoff_seconds=backoff_seconds)
