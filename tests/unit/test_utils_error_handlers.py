...
class TestRecoveryStrategies:
    ...
    def test_recover_configuration_error(self, mock_get_logger):
        ...
        mock_logger.info.assert_called_with(
            "Attempting to load default configuration from /default/config.yaml"
        )
    ...
    def test_recover_external_service_error(self, mock_get_logger):
        ...
        mock_logger.warning.assert_called_with(
            "External service error detected, consider fallback mechanisms"
        )
...
class TestErrorContext:
    ...
    def test_error_context_success(self, mock_get_logger):
        ...
        mock_logger.debug.assert_any_call(
            "Starting operation: test_operation", extra={"context": {"param1": "value1"}}
        )
...
class TestAsyncErrorContext:
    ...
    async def test_async_error_context_success(self, mock_get_logger):
        ...
        mock_logger.debug.assert_any_call(
            "Starting async operation: async_test_operation",
            extra={"context": {"param1": "value1"}},
        )
        mock_logger.debug.assert_any_call(
            "Async operation completed successfully: async_test_operation"
        )
    ...
    async def test_async_error_context_with_error(self, mock_get_logger):
        ...
        mock_logger.error.assert_called_with(
            "Unrecoverable error in async operation: async_test_operation"
        )
    ...
    async def test_async_error_context_with_recovery(self, mock_get_logger):
        ...
        mock_logger.info.assert_called_with(
            "Error recovered in async operation: async_test_operation"
        )
