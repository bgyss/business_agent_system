...
class TestEnhancedInventoryAgentAdditionalCoverage:
    ...
    @pytest.mark.asyncio
    async def test_perform_demand_forecasting_with_declining_items(self, enhanced_inventory_agent, mock_db_session):
        ...
        with patch.object(enhanced_inventory_agent, '_forecast_all_items_demand', return_value=forecasts):
    ...
    @pytest.mark.asyncio
    async def test_optimize_reorder_points_with_cost_savings(self, enhanced_inventory_agent, sample_item):
        ...
        with patch.object(enhanced_inventory_agent, '_calculate_optimal_reorder_point', return_value=optimization), \
             patch.object(enhanced_inventory_agent, '_calculate_current_inventory_cost', return_value=500.0), \
             patch.object(enhanced_inventory_agent, '_calculate_current_inventory_cost_by_id', return_value=500.0):
    ...
    @pytest.mark.asyncio
    async def test_analyze_consumption_movement_reorder_needed(self, enhanced_inventory_agent, sample_item):
    ...
    @pytest.mark.asyncio
    async def test_analyze_consumption_movement_critical_reorder(self, enhanced_inventory_agent, sample_item):
    ...
    @pytest.mark.asyncio
    async def test_analyze_receipt_movement_no_overstock_no_forecast(self, enhanced_inventory_agent, sample_item):
        """Test receipt movement analysis without forecast"""
        mock_session = Mock()
        
        # Receipt that doesn't cause overstock, no forecast
        result = await enhanced_inventory_agent._analyze_receipt_movement(mock_session, sample_item, 20, None)
    ...
    async def test_analyze_adjustment_movement_zero_stock(self, enhanced_inventory_agent):
        ...
        result = await enhanced_inventory_agent._analyze_adjustment_movement(mock_session, zero_stock_item, 10, forecast)
    ...
    @pytest.mark.asyncio
    async def test_analyze_reorder_needs_enhanced_with_urgency_scores(self, enhanced_inventory_agent):
        ...
        with patch.object(enhanced_inventory_agent, '_forecast_item_demand', side_effect=mock_forecast_side_effect):
    ...
    async def test_comprehensive_inventory_analysis_no_issues(self, enhanced_inventory_agent):
        """Test comprehensive inventory analysis when no issues are identified"""
        mock_session = Mock()
        
        # Healthy inventory metrics
        mock_session.query.return_value.filter.return_value.count.side_effect = [100, 5, 2]  # total, low stock (5%), overstocked (2%)
        mock_session.query.return_value.filter.return_value.scalar.side_effect = [50000.0, 8000.0]  # total value, high movements
    ...
    @pytest.mark.asyncio
    async def test_bulk_purchase_optimization_negative_savings(self, enhanced_inventory_agent, sample_item):
        ...
        with patch.object(enhanced_inventory_agent, '_calculate_bulk_purchase_optimization', return_value=optimization):
    ...
    @pytest.mark.asyncio
    async def test_bulk_purchase_calculation_infinite_roi(self, enhanced_inventory_agent, sample_item):
        ...
        with patch.object(enhanced_inventory_agent, '_forecast_item_demand', return_value=forecast):
            ...
            if optimization is not None:
                assert optimization.roi_months == float('inf') or optimization.total_cost_savings <= 0
    ...
    @pytest.mark.asyncio
    async def test_supplier_performance_zero_deliveries(self, enhanced_inventory_agent, sample_supplier):
        ...
        performance = await enhanced_inventory_agent._analyze_individual_supplier_performance(mock_session, sample_supplier)
    ...
    async def test_periodic_check_friday_reorder_optimization(self, enhanced_inventory_agent):
        ...
        with patch.object(enhanced_inventory_agent, '_queue_analysis_message') as mock_queue:
            ...
            with patch('agents.inventory_agent_enhanced.datetime') as mock_datetime:
                mock_datetime.now.return_value = Mock(hour=8, minute=15, weekday=lambda: 4)  # Friday
    ...
    async def test_periodic_check_non_trigger_times(self, enhanced_inventory_agent):
        ...
        with patch.object(enhanced_inventory_agent, '_queue_analysis_message') as mock_queue:
            ...
            with patch('agents.inventory_agent_enhanced.datetime') as mock_datetime:
                mock_datetime.now.return_value = Mock(hour=14, minute=30, weekday=lambda: 3)  # Wednesday 2:30 PM
    ...
    def test_z_score_various_service_levels(self, enhanced_inventory_agent):
        ...
        # Test edge cases
        assert enhanced_inventory_agent._get_z_score_for_service_level(0.89) == 1.28  # Closest to 0.90
        assert enhanced_inventory_agent._get_z_score_for_service_level(0.96) == 1.65  # Closest to 0.95
        assert enhanced_inventory_agent._get_z_score_for_service_level(0.991) == 2.33  # Closest to 0.99

    @pytest.mark.asyncio
    async def test_analyze_stock_movement_enhanced_in_movement(self, enhanced_inventory_agent, sample_item):
    ...
    @pytest.mark.asyncio
    async def test_analyze_stock_movement_enhanced_adjustment_movement(self, enhanced_inventory_agent, sample_item):
        ...
        with patch.object(enhanced_inventory_agent, '_forecast_item_demand', return_value=forecast), \
             patch.object(enhanced_inventory_agent, '_analyze_adjustment_movement', return_value=None):
    ...
    async def test_generate_report_with_forecasts(self, enhanced_inventory_agent, mock_db_session):
        ...
        mock_session_instance.query.return_value.filter.return_value.count.side_effect = [50, 8]  # total, low stock
