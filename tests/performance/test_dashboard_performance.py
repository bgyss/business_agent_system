...
class TestDashboardPerformance:
    ...
    def test_concurrent_dashboard_access(self, performance_config, temp_db_path):
        ...
        def dashboard_worker(worker_id: int):
            ...
            try:
                ...
                for _i in range(5):
                    dashboard.get_financial_summary(days=30)
                    dashboard.get_inventory_summary()
                    dashboard.get_hr_summary()
                    dashboard.get_recent_transactions(limit=10)

                end_time = time.time()

                results_queue.put(
                    {"worker_id": worker_id, "duration": end_time - start_time, "operations": 5}
                )
        ...
        assert (
            total_duration < 30
        ), f"Concurrent dashboard operations took too long: {total_duration:.2f}s"
    ...
    def test_multiple_time_periods_performance(self, benchmark, test_dashboard, medium_dataset):
        ...
        for _days, data in results.items():
    ...
    def test_dashboard_rapid_refresh_performance(self, test_dashboard, medium_dataset):
        ...
        for _refresh in range(20):
            ...
            test_dashboard.get_financial_summary(days=30)
            test_dashboard.get_inventory_summary()
            test_dashboard.get_recent_transactions(limit=10)
