...
Focus on data-driven decisions that optimize total inventory cost and business performance."""
    ...
    async def _perform_demand_forecasting(self, session: Session, data: Dict[str, Any]) -> Optional[AgentDecision]:
    ...
    async def _forecast_item_demand(self, session: Session, item_id: str, forecast_days: int) -> Optional[DemandForecast]:
    ...
    async def _forecast_all_items_demand(self, session: Session, forecast_days: int) -> List[DemandForecast]:
    ...
    async def _calculate_revenue_correlation(self, session: Session, item_id: str, movements: List[StockMovement]) -> float:
    ...
    async def _calculate_optimal_reorder_point(self, session: Session, item: Item) -> Optional[OptimalReorderPoint]:
    ...
    async def _analyze_bulk_purchase_opportunities(self, session: Session) -> Optional[AgentDecision]:
    ...
    async def _calculate_bulk_purchase_optimization(self, session: Session, item: Item) -> Optional[BulkPurchaseOptimization]:
    ...
    async def _analyze_expiry_intelligence(self, session: Session, item: Item) -> Optional[ExpiryIntelligence]:
    ...
    async def _analyze_individual_supplier_performance(self, session: Session, supplier: Supplier) -> Optional[SupplierPerformance]:
    ...
    async def _analyze_stock_movement_enhanced(self, session: Session, movement_data: Dict[str, Any]) -> Optional[AgentDecision]:
    ...
    async def _analyze_consumption_movement(self, session: Session, item: Item, quantity: float, forecast: Optional[DemandForecast]) -> Optional[AgentDecision]:
    ...
    async def _analyze_receipt_movement(self, session: Session, item: Item, quantity: float, forecast: Optional[DemandForecast]) -> Optional[AgentDecision]:
    ...
    async def _analyze_adjustment_movement(self, session: Session, item: Item, quantity: float, forecast: Optional[DemandForecast]) -> Optional[AgentDecision]:
    ...
    async def _analyze_reorder_needs_enhanced(self, session: Session) -> Optional[AgentDecision]:
        ...
        try:
            ...
            items_near_reorder = session.query(Item).filter(
                Item.status == ItemStatus.ACTIVE,
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "reorder_percentage": (low_stock_count / total_items * 100) if total_items > 0 else 0
                },
                ...
                "forecast_summary": {
                    "average_accuracy": sum(f.forecast_accuracy for f in forecasts) / len(forecasts) if forecasts else 0,
