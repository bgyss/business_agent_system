...
class InventoryAgent(BaseAgent):
            ...
            if data.get("type") == "stock_movement":
                ...
                if item_id:
                    ...
                    if seasonality:
                        context = {
                            "item_id": item_id,
                            "seasonality": seasonality._asdict()
                        }
                ...
                if item_id:
                    ...
                    if correlations:
                        context = {
                            "item_id": item_id,
                            "correlations": correlations._asdict()
                        }
                        reasoning = await self.analyze_with_claude(
                ...
                if item_id:
                    ...
                    if diversification:
                        context = {
                            "item_id": item_id,
                            "diversification": diversification._asdict()
                        }
                        reasoning = await self.analyze_with_claude(
    ...
    async def predict_expiry_waste(self, session: Session, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if days_to_consume > item.expiry_days:
                ...
                waste_risk = "medium"
                predicted_waste = max(0, current_stock - (daily_consumption * item.expiry_days * 0.9))
                waste_value = predicted_waste * float(item.unit_cost)
            else:
                waste_risk = "low"
                predicted_waste = 0
    ...
    def _generate_waste_minimization_strategies(
        ...
        if daily_consumption > 0:
            reduced_order_qty = max(
                int(daily_consumption * item.expiry_days * 0.8),
                item.minimum_stock
            )
    ...
    async def analyze_supplier_diversification(
        ...
        try:
            ...
            risk_factors = {
                ...
                'supplier_count': min(1.0, len(supplier_volumes) / 3),  # Fewer suppliers = higher risk
    ...
    def _calculate_cost_competitiveness(
        ...
        for item_id in supplier_items:
            ...
            if item:
                ...
                recent_orders = session.query(PurchaseOrder).join(PurchaseOrderItem).filter(
                    and_(
                        PurchaseOrderItem.item_id == item_id,
                        PurchaseOrder.supplier_id != supplier.id,
    ...
    async def _perform_demand_forecast_analysis(self, session: Session) -> Optional[AgentDecision]:
        ...
        try:
            ...
            high_value_items = session.query(Item).filter(
                and_(
                    Item.status == ItemStatus.ACTIVE,
        ...
        try:
            ...
            recommendation = {
                ...
                "analytics_summary": {
                    ...
                    "seasonality_detected": seasonality is not None and seasonality.confidence > 0.5,
                    "correlations_identified": correlations is not None and len(correlations.correlated_items) > 0,
            ...
            if supplier_diversification and supplier_diversification.risk_score > 0.5:
                recommendation["risk_factors"].append({
                    ...
                    "severity": "high" if supplier_diversification.risk_score > 0.7 else "medium",
