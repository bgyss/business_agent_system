...
class HRAgent(BaseAgent):
    def __init__(self, agent_id: str, api_key: str, config: Dict[str, Any], db_url: str):
        ...
        self.max_labor_cost_percentage = config.get(
    ...
    async def _analyze_time_record(
        self, session, record_data: Dict[str, Any]
    ...
    async def _perform_daily_labor_analysis(self, session) -> Optional[AgentDecision]:
        ...
        yesterday_records = (
            session.query(TimeRecord).filter(func.date(TimeRecord.timestamp) == yesterday).all()
        )
        ...
        labor_cost_percentage = (
            total_labor_cost / estimated_daily_revenue if estimated_daily_revenue > 0 else 0
        )
    ...
    async def _check_overtime_patterns(self, session) -> Optional[AgentDecision]:
        ...
        week_records = (
            session.query(TimeRecord).filter(func.date(TimeRecord.timestamp) >= start_date).all()
        )
        ...
        for employee_id, daily_records in employee_daily_hours.items():
            ...
            for _date, records in daily_records.items():
                daily_hours = self._calculate_daily_hours(records)
                if daily_hours > 8:
    ...
    async def _analyze_staffing_needs(
        self, session, revenue_data: Optional[Dict[str, Any]]
    ) -> Optional[AgentDecision]:
        ...
        upcoming_schedules = (
            ...
            .filter(and_(Schedule.work_date >= start_date, Schedule.work_date <= end_date))
            .all()
        )
    ...
    async def _analyze_leave_request(
        self, session, request_data: Dict[str, Any]
    ) -> Optional[AgentDecision]:
        ...
        while current_date <= end_date:
            ...
            other_schedules = (
                session.query(Schedule)
                .filter(
                    and_(Schedule.work_date == current_date, Schedule.employee_id != employee_id)
                )
                .count()
            )
            ...
            affected_dates.append(
                {
                    ...
                    "has_conflicts": len(
                        [s for s in conflicting_schedules if s.work_date == current_date]
                    )
                    > 0,
                }
            )
        ...
        context = {
            ...
            "minimum_staffing_concern": any(
                date["other_employees_scheduled"] < 2 for date in affected_dates
            ),
        }
        ...
        if leave_type == LeaveType.SICK or leave_type == LeaveType.EMERGENCY:
            ...
            reasoning_prompt = (
                f"Emergency/sick leave request from {employee.first_name} {employee.last_name} "
            ...
            if context["minimum_staffing_concern"] or len(conflicting_schedules) > 2:
                ...
                reasoning_prompt = (
                    f"Leave request from {employee.first_name} {employee.last_name} "
    ...
    async def generate_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_employees = (
                session.query(Employee).filter(Employee.status == EmployeeStatus.ACTIVE).count()
            )

            # Get time records for the period
            time_records = (
                session.query(TimeRecord)
                .filter(func.date(TimeRecord.timestamp) >= start_date)
                .all()
            )
            ...
            pending_leave_requests = (
                session.query(LeaveRequest).filter(LeaveRequest.status == "pending").count()
            )
    ...
    async def _get_current_alerts(self, session) -> List[Dict[str, Any]]:
        ...
        week_records = (
            session.query(TimeRecord).filter(func.date(TimeRecord.timestamp) >= week_start).all()
        )
