...
class FinancialDataGenerator:
    ...
    def generate_daily_transactions(self, date: datetime) -> List[Dict[str, Any]]:
        ...
        daily_revenue_actual = random.normalvariate(
            daily_revenue_target, daily_revenue_target * self.profile.revenue_variance
        )
        ...
        while revenue_generated < daily_revenue_actual:
            transaction_amount = max(
                ...
                random.normalvariate(
                    self.profile.avg_transaction_size, self.profile.avg_transaction_size * 0.3
                ),
            )
    ...
    def _get_vendor_for_category(self, category: str) -> str:
        matching_vendors = [
            name for name, info in self.vendors.items() if info["category"] == category
        ]
        if matching_vendors:
    ...
    def generate_anomalies(
        self, transactions: List[Dict[str, Any]], anomaly_rate: float = 0.02
    ) -> List[Dict[str, Any]]:
        ...
        for transaction in selected_transactions:
            ...
            if anomaly_type == "unusual_amount":
                ...
                anomalous_transaction["transaction_date"] = transaction[
                    "transaction_date"
